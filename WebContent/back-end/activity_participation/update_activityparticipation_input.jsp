...
<title>活動紀錄資料修改 - update_activityparticipation_input.jsp</title>
			...
			<div class="col">
				...
				<FORM METHOD="post"
					...
					<table>
						...
						<jsp:useBean id="PSVO" scope="page"
							class="com.actparticipation.model.ParticipationService" />
						<tr>
						</tr>
						<tr>
							<td>活動報名編號<font color=red><b>*</b></font></td>
							<td><select size="1" name="avPartno">
									<c:forEach var="actPSO" items="${PSVO.all}">
										<option value="${actPSO.avPartno}"
											${(PVO.avPartno==actPSO.avPartno)? 'selected':'' }>${actPSO.avPartno}
									</c:forEach>
							</select></td>
						</tr>
						<tr>
							...
							<td><input type="TEXT" name="memno" size="45"
								value="<%=(PVO == null) ? "" : PVO.getMemno()%>" /></td>
						</tr>
						<tr>
						...
						<tr>
							<td>報名時間</td>
							<td><input type="TEXT" name="actPatTime" id="f_date1" /></td>
						</tr>
						<tr>
							<td>報名人數</td>
							<td><input type="TEXT" name="actParEnr" size="45"
								value="<%=(PVO == null) ? "" : PVO.getActParEnr()%>" /></td>
						</tr>
						<tr>
							<td>報名費用</td>
							<td><input type="TEXT" name="actTalFee" size="45"
								value="<%=(PVO == null) ? "" : PVO.getActTalFee()%>" /></td>
						</tr>
					</table>
					<br> <input type="hidden" name="action" value="update">
					<input type="hidden" name="avPartno"
						value="<%=(PVO == null) ? "" : PVO.getAvPartno()%>"> <input
